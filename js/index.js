...
// Function to handle the "Add Google Account" button click
function handleAuthClick() {
    tokenClient = google.accounts.oauth2.initTokenClient({
        client_id: CLIENT_ID, // Replace with your Google API Client ID
...
if (existingAccount) {
    // Update existing account with the latest token & storage
...
// Function to update storage info once on page load
async function updateStorageOnLoad() {
    for (let account of accounts) {
        if (account.accessToken) {
            const updatedStorage = await getStorageInfo(account.accessToken);
            if (updatedStorage) {
                account.storage = updatedStorage;
            }
        }
    }
    localStorage.setItem('gdrive_accounts', JSON.stringify(accounts));
...
// Run storage update **only** when the page loads or reloads
